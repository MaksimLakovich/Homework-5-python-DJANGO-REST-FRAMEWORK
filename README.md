...
3) Сделать так, что если в запросе метод оплаты = cash то не запускалась интеграция со stripe


4) A. Сериализатор PaymentsSerializer
- Сделать stripe_* и payment_url только для чтения (read_only=True);
- Добавить validate() — чтобы проверять, что указан или курс, или урок, но не оба сразу.


5) B. View: PaymentsListCreateAPIView
