...
2) Сериализатор `PaymentsSerializer(serializers.ModelSerializer)` - класс-сериализатор с использованием класса ModelSerializer для осуществления базовой сериализация в DRF на основе модели Payments. Описывает то, какие поля модели Payments будут участвовать в сериализации и десериализации.
   - Кастомизация сериализатора:
     - функция `def validate(self, data)` - валидирует данные для лучшего API-обслуживания - это ранняя валидация еще в сериализаторе поэтому из контроллера PaymentsListCreateAPIView() я перенес сюда ряд проверок по выбранным Урокам и Курсам.
     Обоснование:
...
1) Сделать так, что если в запросе метод оплаты = cash то не запускалась интеграция со stripe


2) B. View: PaymentsListCreateAPIView
- Переопределить метод create():
- Получить входные данные (курс/урок, сумму);
- Вызвать create_product(), create_price(), create_session();
- Сохранить stripe_* и payment_url в Payments;
- Вернуть сериализованные данные с полем payment_url.
